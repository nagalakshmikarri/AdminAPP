...
class ReferBusinessActivity : AppCompatActivity() {
    private var _binding: ActivityReferBusinessBinding? = null
    private val binding get() = _binding!!
    private var indicatorWidth = 0

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        _binding = ActivityReferBusinessBinding.inflate(layoutInflater)
    ...
    private fun setupUI() {
        binding.tabLayout.addTab(binding.tabLayout.newTab().setText("Refer a business"))
        binding.tabLayout.addTab(binding.tabLayout.newTab().setText("Confirm List"))
        binding.tabLayout.addTab(binding.tabLayout.newTab().setText("Block List"))
        binding.tabLayout.tabGravity = TabLayout.GRAVITY_FILL

        binding.viewPager.isSaveEnabled = false
        val adapter = ReferBusinessAdapter(supportFragmentManager)
