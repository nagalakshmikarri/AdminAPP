...
public class TypesSpinnerAdapter extends ArrayAdapter<TypeListData> {

    private boolean hint;
    private int selectedId = -1;
    private List<TypeListData> items;
    private OnClickListener listener;

    public TypesSpinnerAdapter(Context context, boolean hint, List<TypeListData> items,
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        if (hint) {
            if (position == 0) {
                if (items.get(position).getTyp_title().contains("|")) {
                    holder.etTitle.setHint(items.get(position).getTyp_title().split("\\|")[0]);
                } else {
                    holder.etTitle.setHint(items.get(position).getTyp_title());
                }
            } else {
                ...
                if (items.get(position).getTyp_title().contains("|")) {
                    holder.etTitle.setText(items.get(position).getTyp_title().split("\\|")[0]);
                } else {
                    holder.etTitle.setText(items.get(position).getTyp_title());
                }
            }
        } else {
            if (items.get(position).getTyp_title().contains("|")) {
                holder.tvTitle.setText(items.get(position).getTyp_title().split("\\|")[0]);
            } else {
                holder.tvTitle.setText(items.get(position).getTyp_title());
            }
        }
    ...
    public View getDropDownView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        if (hint && position == 0) {
            ...
            if (items.get(position).getTyp_title() != null) {
                if (items.get(position).getTyp_title().contains("|")) {
                    textview.setText(items.get(position).getTyp_title().split("\\|")[0]);
                } else {
                    textview.setText(items.get(position).getTyp_title());
                }
            }
    ...
    public List<TypeListData> getList() {
        return items;
    }
